...
begin
    ...
    function solve_direct(grid::KoppGrid{Dim}, topology, dh, refinement_cache, sync, dt, io_enabled::Bool=false) where Dim
        ...
        for T in 0:0.1:T_end
            ...
            @time "pre" @views for cc in CellIterator(grid)
            ...
            teprev = copy(te)
            u_prev = copy(u)
            @info extrema(values(dt))
            # @views while any(t_el -> t_el < T, te)
            @views while last(peek(te_pq)) < T
                u_new .= 0.0
                ...
                uhelp = zeros(size(nMinvK_local, 2))
                ...
                u_new[dofs_temp_storage] .= u[dofs_temp_storage] + Δt * nMinvK_local * uhelp
                # end
